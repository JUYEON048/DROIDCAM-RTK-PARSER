...
class RTK_Class(object):
    def __init__(self):
        port = "/dev/ttyUSB0" #IONIQ차량에서 사용할 때는 serial port "/dev/ttyRTK"로 지정
        self.serialPort = serial.Serial(port, baudrate = 115200, timeout = 0.1) #115200
...
if __name__=='__main__':
    ...
    rc = RTK_Class()
    #rospy.Timer(rospy.Duration(1.0/2000.0), rc.parsing)
    rospy.spin()
